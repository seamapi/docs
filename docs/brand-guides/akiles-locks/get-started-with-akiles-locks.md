...
You can use the Seam API to program [online access codes](https://docs.seam.co/latest/low-level-apis/smart-locks/access-codes) on Akiles locks that have an integrated keypad or an Akiles Pinpad. Lock users can then enter these access codes using the keypad or Pinpad to unlock the lock.
