...
{% hint style="success" %}
Because you're using a sandbox workspace, you can connect Seam's test TTLock account. We provide [virtual devices](../developer-tools/sandbox-and-sample-data/) for each of the brands that we support. These sandbox devices and systems enable you to test your app with devices from multiple brands without the need to own all the corresponding physical devices.
{% endhint %}

Complete the Connect Webview authorization flow by entering the following [TTLock sandbox account](../developer-tools/sandbox-and-sample-data/ttlock-sample-data.md) credentials:
