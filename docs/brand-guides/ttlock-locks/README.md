...
1.  Create an account in the [TTLock app](https://www.ttlock.com/#/lock/app) if you have not done so already.
    ...
    **Note:** Your users must be the top administrator for each TTLock that they want to connect to Seam.
...
6. Use your credentials for the TTLock app to log in to the [Seam Connect Webview](https://docs.seam.co/latest/core-concepts/connect-webviews) to add your devices to Seam.
...
Your users must be the top administrator for each TTLock that they want to connect to Seam. That is, they must supply the credentials for the top administrator account when completing the [Connect Webview](https://docs.seam.co/latest/core-concepts/connect-webviews) to connect their TTLocks to Seam.

### Lock Not Paired to Gateway Error

On the device, if you notice a `ttlock_lock_not_paired_to_gateway` error, you will need to purchase [a compatible gateway](https://ttlock.eu/shop/tag/ttlock+gateway/) and pair it to the lock. Follow the [set up instructions](README.md#set-up-instructions) to configure the door lock.

### Gateway Unlocking Not Enabled Warning

On the device, if you notice a `ttlock_lock_gateway_unlocking_not_enabled` warning, you will need to follow the [set up instructions](README.md#set-up-instructions) to turn on the "Remote Unlock" feature.
