...
A [thermostat](https://docs.seam.co/latest/capability-guides/thermostats) was adjusted manually. The event is triggered when a thermostat's climate settings are changed by the user (either through the device itself or via the Seam API), and these changes differ from the stored climate presets.
