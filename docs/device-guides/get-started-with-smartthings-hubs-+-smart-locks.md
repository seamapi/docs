...
When you link an SmartThings account with Seam, we create a `device` object to represent each SmartThings-connected lock in your account. You can then retrieve these SmartThings devices using the [List Devices](../api-clients/devices/list.md) and [Get Device](../api-clients/devices/get.md) endpoints.
...
// Retrieve all devices, filtered by manufacturer,
// which is one of several filters that list() supports.
var allSmartThingsLocks = seam.devices().list(DevicesListRequest.builder()
  .manufacturer(Manufacturer.SMARTTHINGS)
  .build());

// Select the first device as an example.
Device frontDoor = allSmartThingsLocks.get(0);
...
// Retrieve all devices, filtered by manufacturer,
// which is one of several filters that list() supports.
allSmartThingsLocks, err := client.Devices.List(
  context.Background(), &api.DevicesListRequest{
    Manufacturer: api.ManufacturerSmartthings.Ptr(),
  },
)

// Select the first device as an example.
frontDoor := allSmartThingsLocks[0]
