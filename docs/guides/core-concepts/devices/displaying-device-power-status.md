...
When issuing a [List Events](https://docs.seam.co/latest/api/events/list) request to retrieve [`device.low_battery`](https://docs.seam.co/latest/api/events/) or [`device.battery_status_changed`](https://docs.seam.co/latest/api/events/) events for a specific device, include the following parameters:
