...
1. Execute [`/connect_webviews/create`](https://docs.seam.co/latest/api/connect_webviews/create) and specify the internal ID as a key:value pair in the `custom_metadata`.\
   For example:
...
5. To retrieve the ID of the connected account associated with the completed Connect Webview, [get the `connect_webview`](https://docs.seam.co/latest/api/connect_webviews/get) and check the `connected_account_id` property.
6. To retrieve the connected account, [get the `connected_account`](https://docs.seam.co/latest/api/connected_accounts/get) with the ID that you identified in the previous step.
