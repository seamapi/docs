...
For detailed information about installing the Seam Bridge and connecting an on-premises ACS to Seam, see [Seam Bridge](../../../capability-guides/seam-bridge.md).
...
In a credential-based ACS, the [`acs_credential`](https://docs.seam.co/latest/api/acs/credentials/) resource contains the list of entrances to which the credential grants access. The `acs_credential` also contains the schedule that specifies when this access is valid. You specify each schedule by configuring a `starts_at` and `ends_at` date and time.
...
2. Create ACS users.\
   Your application [creates](../../../low-level-apis/access-systems/user-management.md#create-a-user) an [`acs_user`](https://docs.seam.co/latest/api/acs/users/) for each user within this ACS. Each `acs_user` is configured with a set of attributes, such their name, email address, and phone number, as applicable.\
   If you need to grant a single application user access to multiple ACSs—for example, if an application user needs access to multiple buildings, each of which uses a separate ACS—you can use Seam [user identities](https://docs.seam.co/latest/api/user_identities/) to link `acs_user`s in different `acs_system`s.
