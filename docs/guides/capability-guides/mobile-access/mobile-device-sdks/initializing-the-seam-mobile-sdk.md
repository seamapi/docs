...
{% hint style="info" %}
You must also use the Seam API to perform server-side actions. Consequently, [install a Seam server-side SDK](https://docs.seam.co/latest/api/installation) in the language of your choice if you have not done so already.
{% endhint %}
