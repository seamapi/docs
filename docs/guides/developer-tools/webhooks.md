...
You can configure endpoints in each workspace for an app to receive webhook [event](https://docs.seam.co/latest/api/events/) notifications for resources, such as devices and connected accounts.

To configure webhook endpoints:

1. In the left navigation pane of the [Seam Console](https://console.seam.co/), click **Webhooks**.
2. Click **+ Add Webhook**.
3. In the **Create Webhook** dialog, type your webhook URL and select the [event types](https://docs.seam.co/latest/api/events/) for which you want to receive events.
4. Click **Create**.
...
Webhook endpoints can receive many different types of events, such as `device.connected` or `device.low_battery`. It's a good idea when getting started with webhooks to review the list of webhook [event types](https://docs.seam.co/latest/api/events/) and test your endpoint using the Svix sandbox environment.
