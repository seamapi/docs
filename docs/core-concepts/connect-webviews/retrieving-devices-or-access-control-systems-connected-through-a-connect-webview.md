...
{% hint style="info" %}
If you set `wait_for_device_creation` to `false` when [creating the Connect Webview](./#id-1.-create-a-connect-webview), you should wait for the [`connected_account.completed_first_sync` event](../../api/events/#event-types) before retrieving the user's ACS. This event indicates that Seam has finished the first sync of the connected account and the `acs_system` is now available.
{% endhint %}
