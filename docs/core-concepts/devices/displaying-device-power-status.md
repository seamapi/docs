...
You can retrieve these events using a [List Events](../../api/events/list.md) request or through [webhooks](../webhooks.md) and then display the corresponding status in your app.

### Get Battery-Related Events Using a List Events Request

When issuing a [List Events](../../api/events/list.md) request to retrieve [`device.low_battery`](../../api/events/#event-types) or [`device.battery_status_changed`](../../api/events/#event-types) events for a specific device, include the following parameters:
