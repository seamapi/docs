---
title: 'Assigning ACS Users to Access Groups'
description: 'Learn how to use access groups to manage ACS user permissions across different doors and areas.'
---
...
<Info>
  To learn whether your ACS supports access groups, see the [system integration
...
To [add an ACS user to an access group](/api/acs/users/add_to_access_group), provide both the `acs_user_id` and the `acs_access_group_id`.
...
To [remove an ACS user from an access group](/api/acs/users/remove_from_access_group), provide both the `acs_user_id` and the `acs_access_group_id`.
