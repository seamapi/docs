---
title: 'Troubleshooting Access Codes'
...
When scheduling an access code on a device that supports [native scheduling](/low-level-apis/smart-locks/access-codes#native-scheduling), Seam begins to load it onto the device three days in advance, unless it's a [just-in-time code](/low-level-apis/smart-locks/access-codes#native-scheduling). Seam records any errors or warnings on the access code when it attempts to set it on the device. This approach ensures that you have ample time to address any issues, using the mitigation steps in this guide. Note that not all device brands support the ability to program codes in advance. See [Native Scheduling—Supported Providers](/low-level-apis/smart-locks/access-codes#native-scheduling-supported-providers) for details.
...
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `failed_to_set_on_device`                          | Check the error message for more specific mitigation steps or use a [backup access code](/low-level-apis/smart-locks/access-codes/backup-access-codes)                                                                                                                                                                                                                                                                                             |
...
| `duplicate_code_on_device`                         | Check for [unmanaged access codes](/low-level-apis/smart-locks/access-codes/migrating-existing-access-codes) through the provider's app or Seam with the same pin code or name. If a duplicate is found, either delete the duplicate code or convert it to a managed code.                                                                                                                                                                         |
...
| `kwikset_unable_to_confirm_code`                   | This error is usually caused by a duplicate pin code on the device that was set outside of Seam. Try to update this access code's 'code' value through Seam Console or by using our [Update Access Code](/api/access_codes/update) endpoint                                                                                                                                       |
