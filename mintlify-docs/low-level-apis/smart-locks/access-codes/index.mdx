...
An access code is a code used for a keypad or pinpad device. Unlike physical keys, which can easily be lost or duplicated, PIN codes can be customized, tracked, and altered on the fly. Using the Seam [Access Code API](/api/access_codes/object), you can easily generate access codes on the hundreds of door lock models with which we integrate.
...
For [igloohome locks](/device-and-system-integration-guides/igloohome-locks) and [Lockly locks](/device-and-system-integration-guides/lockly-locks), you can create [one-time-use offline access codes](/low-level-apis/smart-locks/access-codes/offline-access-codes#creating-one-time-use-offline-access-codes). igloohome one-time-use offline access codes are valid for 24 hours from the `starts_at` timestamp that you configure. Lockly offline access codes are valid for the time period that you specify using the `starts_at` and `ends_at` timestamps. These codes expire after a single use.
