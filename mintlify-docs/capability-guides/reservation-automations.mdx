...
1. You create spaces and assign devices using [`/spaces/create`](/api/spaces/create).
...
- [Customer](/capability-guides/customer-portals/index) – identify who the automation belongs to with a `customer_key`.
- [Spaces](../core-concepts/mapping-your-resources-to-seam-resources) – represent the real-world units your customer manages (i.e. _Room 101_ in a hotel, _Studio 3_ in a gym). Each space must be created via [`/spaces/create`](/api/spaces/create) with a `space_key` and assigned devices or entrances **before** you call `push_data`. Reservations reference these spaces by `space_key`.
...
<Warning>
  ...
  If you skip [`/spaces/create`](/api/spaces/create),
...
<Check>
  ...
  with [Customer Portals](/capability-guides/customer-portals/index).
</Check>
...
Before pushing reservation data, create a space for each bookable unit using the [`/spaces/create`](/api/spaces/create) endpoint. Each space must have a `space_key` (your identifier) and at least one assigned device or entrance.
...
<img src="/images/Screenshot-2026-03-31-at-23.46.11.png" alt="" style={{width: "550px", height: "auto"}} />
...
<Info>
  The `push_data` [API
  reference](/api/customers/push_data) also documents
  ...
  [`delete_data`](/api/customers/delete_data).
</Info>
...
| Symptom                      | Cause                                            | Fix                                                                                                                                                                         |
| ---------------------------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No access code created       | Space does not exist or is missing a `space_key` | Create the space via [`/spaces/create`](/api/spaces/create) with a `space_key` and `device_ids` or `acs_entrance_ids` before calling `push_data` |
