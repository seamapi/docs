...
Method by which the affected lock device was unlocked. When the method is `keycode`, the `access_code_id` indicates the [access code](/low-level-apis/smart-locks/access-codes) that was used, if reported by the device.
