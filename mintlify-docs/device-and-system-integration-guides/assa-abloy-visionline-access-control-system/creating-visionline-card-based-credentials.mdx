...
2. Create a [credential](https://docs.seam.co/latest/low-level-apis/access-systems/managing-credentials#create-a-card-based-credential) with the `access_method` set to `card` and the appropriate [`visionline_metadata`](./mobile-credential-related-properties#acs_credential.visionline_metadata-properties), such as the card format.
