---
title: 'Get started with igloohome Locks'
...
<Info>
  ...
  you need to connect a real Igloohome device, use a non-sandbox workspace and
  API key.
</Info>
...
Navigate to the URL returned by the Webview object. Since you are using a sandbox workspace, complete the login flow by entering the Igloohome [sandbox test accounts ](igloohome-sample-data)credentials below:
...
<Info>
  Igloohome pins have to be 4-6 digits long, and for time bound access codes,
  start date can't be on the past.
</Info>
