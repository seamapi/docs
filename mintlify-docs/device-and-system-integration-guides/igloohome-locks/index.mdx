...
<img
  src="https://strapi-media-bucket.fly.storage.tigris.dev/sq_igloohome_906ca2ea02.svg"
  alt="igloohome logo"
  width="192"
  height="192"
...
igloohome devices have an auto-relock feature that automatically locks the device after a configurable period of time, for example, 30 seconds. The Seam API reports these auto-relock occurrences by emitting a [`lock.locked` event](/api/events/object) with `automatic` as the value for the `method` property.
