...
When you link a Honeywell Resideo account with Seam, we create a `device` object to represent each Honeywell Resideo thermostat in your account. You can then retrieve these Honeywell Resideo devices using the [List Devices](/api/devices/list) and [Get Device](/api/devices/get) endpoints.
...
Now that you have learned the basics of using the Seam API, you can connect and control a real Honeywell Resideo device. To do so, make sure to switch to a [non-sandbox workspace](/core-concepts/workspaces#production-workspaces) and [API key](/core-concepts/authentication/api-keys).
