---
title: 'Get started with 4SUITES locks'
...
<Info>
  ...
  you want to connect a real 4SUITES lock, use a [non-sandbox
...
<Check>
  Because you're using a sandbox workspace, you can connect Seam's test 4SUITES
  account. We provide [virtual devices](./4suites-locks-sample-data) for each
  of the brands that we support. These sandbox devices and systems enable you to
  test your app with devices from multiple brands without the need to own all
  the corresponding physical devices.
</Check>

Complete the Connect Webview authorization flow by entering the following [4SUITES sandbox account](./4suites-locks-sample-data) credentials:
...
When you link a 4SUITES account with Seam, we create a `device` object to represent each 4SUITES lock in your account. You can then retrieve these 4SUITES devices using the [List Devices](/api/devices/list) and [Get Device](/api/devices/get) endpoints.
...
const allFourSuitesLocks = await seam.devices.list({
...
Now that you have learned the basics of using the Seam API, you can connect and control a real 4SUITES device. To do so, make sure to switch to a [non-sandbox workspace](/core-concepts/workspaces#production-workspaces) and [API key](/core-concepts/authentication/api-keys).
