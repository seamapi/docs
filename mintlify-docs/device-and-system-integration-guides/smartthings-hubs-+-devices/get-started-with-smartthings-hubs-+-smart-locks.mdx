---
title: 'Get Started with SmartThings Hubs + Smart Locks'
description: 'Learn how to connect and control SmartThings-connected locks with the Seam API.'
---
...
<Info>
  ...
  you want to connect a real SmartThings-connected lock, use a [non-sandbox
...
When you link an SmartThings account with Seam, we create a `device` object to represent each SmartThings-connected lock in your account. You can then retrieve these SmartThings devices using the [List Devices](/api/devices/list) and [Get Device](/api/devices/get) endpoints.
...
const allSmartThingsLocks = await seam.devices.list({
...
You can use the Seam API to program [online access codes](/low-level-apis/smart-locks/access-codes) on SmartThings-connected locks that have an integrated or accessory keypad. Lock users can then enter these access codes using the keypad to unlock the lock.
