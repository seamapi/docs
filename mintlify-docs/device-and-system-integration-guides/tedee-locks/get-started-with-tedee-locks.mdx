...
Once installed, [sign up for Seam](https://console.seam.co/) to get your [API key](/core-concepts/authentication/api-keys) and export it as an environment variable.
...
The Seam API makes it easy to program both `ongoing` codes and `timebound` codes on a Tedee lock. You can find out more about Tedee lock access code in our [core concept section on access codes.](/low-level-apis/smart-locks/access-codes)

<Info>
  For Tedee-specific access code restrictions, see [Access (PIN) Code
  Requirements](/device-and-system-integration-guides/tedee-locks#brand-specific-access-pin-code-requirements).
</Info>
