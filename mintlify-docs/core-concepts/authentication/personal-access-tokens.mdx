...
description: 'Create a Personal Access Token to access multiple Seam workspaces using a single credential.'
---

In some instances, it may be necessary to access multiple workspaces with a single Seam credential. For example, you may want to set up separate workspaces for each of your users to control different sets of devices. For more information about scenarios in which multiple workspaces are useful, see [Per-Customer Workspace Setup](/core-concepts/workspaces/index#per-customer-workspace-setup-less-common).

Because [API keys](/core-concepts/authentication/api-keys) are scoped to a single workspace, Seam provides Personal Access Tokens (PATs) to enable multi-workspace authorization. First, [create a PAT](#create-a-personal-access-token). Then, [use this PAT with a workspace ID](#using-the-pat-to-issue-a-seam-api-request) to access the desired workspace.

<Warning>Never use a PAT in a web browser or expose it to your users.</Warning>

<img src="/images/personal-access-token.png" alt="Unlike API keys, PATs enable multi-workspace authorization." style={{width: "460px", height: "auto"}} />
...
The following example shows how to use a PAT and workspace ID to [list all devices](/api/devices/list) in a workspace:
