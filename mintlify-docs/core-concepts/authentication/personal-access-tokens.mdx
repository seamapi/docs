...
Because [API keys](api-keys) are scoped to a single workspace, Seam provides Personal Access Tokens (PATs) to enable multi-workspace authorization. First, [create a PAT](personal-access-tokens#create-a-personal-access-token). Then, [use this PAT with a workspace ID](personal-access-tokens#use-a-pat-and-workspace-id-in-the-seam-api) to access the desired workspace.
