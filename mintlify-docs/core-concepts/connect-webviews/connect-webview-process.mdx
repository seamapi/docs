...
In your app, [create a Connect Webview](/api/connect_webviews/create). The Seam API represents Connect Webviews as `connect_webview` objects.
...
- To display a preselected group of providers, you can specify the desired `provider_category`. Supported provider categories are as follows:
  ...
  To list all providers within a category, use the [List Device Providers](/api/devices/list_device_providers) method with the desired `provider_category` filter.
