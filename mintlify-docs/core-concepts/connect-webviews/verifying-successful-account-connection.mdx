---
title: 'Verifying Successful Account Connection'
description: "Learn how to find out when your user's device or ACS account has connected successfully to Seam."
---
...
- If you've redirected your app to the Connect Webview, [poll](#polling) or use a [webhook](#webhook) to determine when the Connect Webview has completed successfully. You can then [retrieve the device or ACS](/core-concepts/connect-webviews/retrieving-devices-or-access-control-systems-connected-through-a-connect-webview) associated with the newly-connected account.
- If you've embedded the Connect Webview in an [iframe](https://www.w3schools.com/html/html_iframe.asp), use [event listening](#event-listening) to determine when to close the iframe. You can then [retrieve the device or ACS](/core-concepts/connect-webviews/retrieving-devices-or-access-control-systems-connected-through-a-connect-webview) associated with the newly-connected account.
...
For more information, see [Webhooks](../../developer-tools/webhooks).
...
When you [host a Connect Webview in an HTML iframe](/core-concepts/connect-webviews/embedding-a-connect-webview-in-your-app#embed-the-connect-webview-in-an-iframe), the [iframe](https://www.w3schools.com/html/html_iframe.asp) uses `window.parent.postMessage` to send messages to the parent window that is hosting the iframe. `window.parent.postMessage` is a cross-origin communication mechanism available in web browsers.
...
| Event                                    | Description                                                                                                                                                                                                                                                                                                                |
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `connected_account.created`              | A [connected account](/core-concepts/connected-accounts/index) has been imported into your Seam workspace.                                                                                                                                                                                                                                  |
| `connected_account.connected`            | Seam has established connection to or is reconnected to a [connected account](/core-concepts/connected-accounts/index).                                                                                                                                                                                                                     |
