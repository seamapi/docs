---
title: 'Embedding a Connect Webview in Your App'
description: 'Use redirection or an iframe to embed a Connect Webview in your app.'
---

<figure>
  <img
    src='/images/connect-webview-embedding-2.png'
    alt='Seam makes it easy for your users to authorize your app to control their devices using the Seam API.'
  />
  <figcaption></figcaption>
</figure>

To enable your users to connect their devices or access control system to Seam through your app, use a [Connect Webview](./). First, in your app, add a mechanism—such as a button—to [open a new Connect Webview](#step-1-open-a-connect-webview). Then, you can use either of the following two methods to [display the Connect Webview](#step-2-display-the-connect-webview) in your app:

- [Redirect to the Connect Webview URL.](#redirect-to-the-connect-webview-url)
- [Embed the Connect Webview in an inline frame (iframe).](#embed-the-connect-webview-in-an-iframe)
...
When your user clicks this button, your app can either [redirect to the Connect Webview URL](#redirect-to-the-connect-webview-url) or [embed the Connect Webview in an iframe](#embed-the-connect-webview-in-an-iframe) and present it in a modal.

<figure>
  <img
    src='/images/connect-webview-embedding-1-(1).png'
    alt='Add a button to your app to enable your users to connect their devices or ACS using a Connect Webview.'
    width='563'
  />
  <figcaption>
    <p>
      Add a button to your app to enable your users to connect their devices or
      ACS using a Connect Webview.
    </p>
  </figcaption>
</figure>
...
You can [redirect to the Connect Webview URL](#redirect-to-the-connect-webview-url) or [use an iframe](#embed-the-connect-webview-in-an-iframe) to display the Connect Webview. The following sections describe these two options:

### Redirect to the Connect Webview URL

To incorporate a Connect Webview in your app using redirection:

1. [Create the Connect Webview.](#step-1-open-a-connect-webview)
...
2. Redirect your app to the URL for the created Connect Webview.\
   ...
   The resulting page displays the [authorization flow](#step-1-open-a-connect-webview) through which your app user can link their device or ACS account to Seam.\\

   <figure>
     <img
       src='/images/connect-webview-user-flow.png'
       alt='The page to which you redirect your app displays the authorization flow.'
     />
     <figcaption></figcaption>
   </figure>
...
3. [Your user completes the Connect Webview authorization flow.](#opening-connect-webviews-in-ios-or-android-apps)
4. [Learn when the device or ACS account connection has completed successfully.](#step-2-display-the-connect-webview)
5. [Retrieve the devices or ACS that your user has just connected to Seam.](#step-2-display-the-connect-webview)
6. [Use the Seam API to control your users' connected devices or ACS.](#step-2-display-the-connect-webview)
...
3. [Your user completes the Connect Webview authorization flow.](#opening-connect-webviews-in-ios-or-android-apps)
   ...
   <Info>
     ...
     understand that some providers move to a new tab during their
     authentication flow for security purposes. Seam is not able to abstract
     away this behavior.{' '}
   </Info>
