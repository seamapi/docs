---
title: 'Displaying Device Power Status'
description: "You can display the power status of your end users' devices in your app."
---

<figure>
  <img
    src='/images/seam-component-device-power-status-display-example.png'
    alt='Display the power status of your end users&#x27; devices in your app.'
  />
...
- [Use the `properties.has_direct_power`, and `properties.battery` properties of the `device` object.](displaying-device-power-status#get-device-power-status-using-device-properties)
- [Use the `device.low_battery` and `device.battery_status_changed` events.](displaying-device-power-status#get-device-power-status-using-battery-related-events)
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <code>properties.has_direct_power</code>
      </td>
      <td>Boolean</td>
      <td>
...
You can retrieve these events using a [List Events](https://docs.seam.co/latest/api/events/list) request or through [webhooks](../../developer-tools/webhooks) and then display the corresponding status in your app.
