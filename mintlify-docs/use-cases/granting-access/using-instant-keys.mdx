...
## Step 3: List the Doors This Instant Key Applies To

An access method can cover multiple doors — for example, if the Access Grant includes several entrances or spaces. To see exactly which doors an Instant Key unlocks, call `/access_methods/get_related` with the underlying mobile key's access method ID.
