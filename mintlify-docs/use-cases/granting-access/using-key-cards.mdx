...
## Step 4: List the Doors This Key Card Applies To

An access method can cover multiple doors — for example, if the Access Grant includes several entrances or spaces. To see exactly which doors a key card unlocks, call `/access_methods/get_related`.
