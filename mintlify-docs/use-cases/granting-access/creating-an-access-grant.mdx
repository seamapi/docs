...
| `card`       | Encode the credential onto a physical card. `is_issued` becomes `true` after encoding. | [Using Key Cards](/use-cases/granting-access/using-key-cards)                |
| `cloud_key`  | Call `access_methods.unlock_door` to unlock on behalf of the user.         | [Using Cloud Keys](/use-cases/granting-access/using-cloud-keys)              |
