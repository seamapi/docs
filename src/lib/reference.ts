...
export const reference = (
  ...
  for (const route of routes) {
    ...
    for (const endpoint of route.endpoints) {
      ...
      Note that the name provided on Seam is used to identify the code on Seam and is not necessarily the name that will appear in the lock provider's app or on the device. This is because lock providers may have constraints on names, such as length, uniqueness, or characters that can be used. In addition, some lock providers may break down names into components such as <code>first_name</code> and <code>last_name</code>.

      To provide a consistent experience, Seam identifies the code on Seam by its name but may modify the name that appears on the lock provider's app or on the device. For example, Seam may add additional characters or truncate the name to meet provider constraints.

      To help your users identify codes set by Seam, Seam provides the name exactly as it appears on the lock provider's app or on the device as a separate property called <code>appearance</code>. This is an object with a <code>name</code> property and, optionally, <code>first_name</code> and <code>last_name</code> properties (for providers that break down a name into components).`,
              }
            }
